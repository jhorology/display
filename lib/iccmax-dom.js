...
module.exports = class IccMaxDOM {
  ...
  selectDisplayChromacity() {
    return ['white', 'red', 'green', 'blue'].reduce((obj, ch) => {
      const tags = {
        white: 'mediaWhitePoint',
        red: 'redColorant',
        green: 'greenColorant',
        blue: 'blueColorant'
      }
  ...
  /**
   * @param {string} ch - 'red', 'green' or 'blue'
   */
  selectToneResponseCurve(ch) {
    let node = this.select(`/IccProfile/Tags/${ch}TRCTag`, true)
    if (!node) {
      return undefined
    }
