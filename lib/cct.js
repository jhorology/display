...
function uv2cct_jis(uv, uvt = JIS_Z8725_UVT) {
    ...
    fndt = (n) =>
      _dt[n] !== undefined
        ...
        : (_dt[n] = distanceFromPointToLine(
            [u, v],
            // normal line of isotherm ax + by + c = 0
            // A = -1 normal vector towords high temperature
            // B = 'rs' of table item
            // C = -ax - by
            { A: -1, B: uvt[n].rs, C: uvt[n].u - uvt[n].rs * uvt[n].v }
          ))
  ...
  const { u: u0, v: v0 } = uvt[n],
    { u: u1, v: v1 } = uvt[n + 1],
    { u: u2, v: v2 } = uvt[n + 2],
    uqi = quadraticInterpolation([n, u0], [n + 1, u1], [n + 2, u]),
...
function uv2cct_jis_mod(uv, uvt = UVT) {
  ...
  while (from <= to) {
    ...
    if (angle >= angle0 && angle <= angle1) {
      ...
      // const rt = uvt.length - mid - ((angle - angle0) / (angle1 - angle0))
      const rt = rt0 + (rt1 - rt0) * ((angle - angle0) / (angle1 - angle0))
...
/**
 * Convert CCT and Duv to CIE 1960 UCS CCT coordinates
...
function cct2uv_jis_mod(cct, duv, uvt = UVT) {
  ...
  while (from <= to) {
    ...
    if (rt <= rt0 && rt >= rt1) {
      ...
      let xd = (radius + duv) * Math.cos(angle),
        yd = (radius + duv) * Math.sin(angle)
      if (yd < 0) {
        xd = -xd
        yd = -yd
      }
      const uv = [center[0] + xd, center[1] + yd]
      return uv2(uv)
    } else if (rt < rt1) {
      from = mid + uvt.length1
    } else if (rt > rt0) {
      to = mid - 1
    }
  }
