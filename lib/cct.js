...
/**
 * Convert CCT to CIE 1931 XY coordinates of daylight illuminant
 * JIS Z 8720
 */
function cct2xy_daylight(t) {
  if (t < 4000 || t > 25000) {
    throw new Error('input value is out of range. t:' + t)
  }
  let k0, k1, k2, k3
  if (t <= 7000) {
    k0 = 0.244063
    k1 = 0.09911 * Math.pow(10, 3)
    k2 = 2.9678 * Math.pow(10, 6)
    k3 = -4.607 * Math.pow(10, 9)
  } else {
    k0 = 0.23704
    k1 = 0.24748 * Math.pow(10, 3)
    k2 = 1.9018 * Math.pow(10, 6)
    k3 = -2.0064 * Math.pow(10, 9)
  }
  const x = k0 + k1 / t + k2 / (t * t) + k3 / (t * t * t),
...
function cct2uv_jis_mod(cct, duv, uvt = UVT) {
  ...
  while (from <= to) {
    ...
    const splitRatio = (rt - rt0) / (rt1 - rt0),
      // If limit the table's rt step to -1 and the value range from table.length - 1 to 0
      // splitRatio = mid - uvt.length - rt
      angle = angle0 + (angle1 - angle0) * splitRatio,
      ud = (radius + duv) * Math.cos(angle),
      vd = (radius + duv) * Math.sin(angle),
      uv = [center[0] + ud, center[1] + vd]
    return uv2(uv)
  }
...
/**
 * Convert CCT to daylight illuminant (CIE D)and Duv to CIE 1960 UCS uv coordinates
 * @param {number} cct - corelated temperature
 * @param {number} duv - distance from blacbody locus, positive is on the greenish side, negative is on the magenta side
 * @param {number[u, v]} uv - CIE 1960 UCS uv coordinates
 * @returns {object} - Object containing uv, xy, xyz
